...
# InfoNCE loss (Oord, Li, and Vinyals 2018) Oord, A. v. d.; Li, Y.; and Vinyals, O. 2018. Representation
...
def dot_similarity(ref: torch.Tensor, pos: torch.Tensor,
    ...
    neg_dist = torch.einsum('ni,nji->nj', ref, neg)
...
def euclidean_similarity(
    ...
    neg_sq = torch.einsum("nji->nj", neg**2)
    ...
    neg_dist = -(ref_sq[:, None] + neg_sq - 2 * neg_cosine)
    return pos_dist, neg_dist
...
def infonce(
    ...
    c = c.detach()
    pos_dist = pos_dist - c
    neg_dist = neg_dist - c.unsqueeze(1)
    align = (-pos_dist)
    uniform = torch.logsumexp(neg_dist, dim=1)
...
class FixedCosineInfoNCE(FixedInfoNCE):
    ...
    def _distance(self, ref: torch.Tensor, pos: torch.Tensor,
        ...
        # return pos_dist / self.temperature, scaling_neg_dist(neg_dist) / self.temperature
